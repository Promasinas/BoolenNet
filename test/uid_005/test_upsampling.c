...
void test_forward(void) { T("upsample_forward (uint8_t)");
    ...
    /* bit=0→pass, bit=1→flip(negate as uint8) */
    C(out[0]==3 && out[1]!=5,"r0: pass[0], flip[1]");
    C(out[2]!=3 && out[3]==5,"r1: flip[0], pass[1]");
    upsample_destroy(u); }
...
void test_cascade(void) { T("upsample_cascade");
    ...
    uint8_t in[3]={1,2,3}, out[6]={0};
    UpsampleLayer *layers[2]={u1,u2};
    C(!upsample_cascade(layers,2,in,out),"cascade ok");
    C(out[0]!=1 && out[1]==2,"double-flip: 1→neg→neg=original?");
