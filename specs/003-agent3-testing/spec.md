...
- Q: 并行编译的默认最大并发进程数是多少？ → A: CPU 核心数——自动检测系统 CPU 核心数（若可检测），设为编译并发上限，确保不超过系统承载能力。
...
**Independent Test**: 当 Agent 3 检测到 `./src/` 中的新模块（如 `bool_router.c`）后，在 `./test/` 下自动创建对应的测试子目录（如 `./test/bool_router/`），目录结构包含测试代码文件占位和 Makefile。

**Acceptance Scenarios**:

1. **Given** `./src/` 中有一个新模块 `bool_router.c`，**When** Agent 3 为该模块创建测试目录，**Then** `./test/bool_router/` 目录被创建，且包含初始的测试文件框架和 Makefile。
...
1. **Given** 测试目录 `./test/bool_router/` 已创建且包含测试源代码，**When** Agent 3 执行编译流程，**Then** 生成正确的 Makefile，使用 `make` 命令成功编译出测试可执行文件。
...
- 当同一个模块在短时间内被多次修改，Agent 3 应能合并或串行化测试请求，避免重复编译和测试竞态。
- 当编译产生的可执行文件过大或运行时间过长时，Agent 3 应设置合理的超时限制，避免无限等待。
